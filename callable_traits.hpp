...
struct callable_traits_impl  // Intermediary for classes and lambdas
{
  ...
  public:
    using return_type = typename callable_traits_impl<decltype(
        getOperator(&T::operator()))>::return_type;
    using args = typename callable_traits_impl<decltype(
        getOperator(&T::operator()))>::args;
    static const size_t args_count =
        callable_traits_impl<decltype(getOperator(&T::operator()))>::args_count;
};
template <class T>
struct callable_traits_impl<const T>  // Intermediary for const classes and
                                      // lambdas
{
  private:
    template <class R, class C = T, class... Args>
  ...
  public:
    using return_type = typename callable_traits_impl<decltype(
        getConstOperator(&T::operator()))>::return_type;
    using args = typename callable_traits_impl<decltype(
        getConstOperator(&T::operator()))>::args;
    static const size_t args_count = callable_traits_impl<decltype(
        getConstOperator(&T::operator()))>::args_count;
};
